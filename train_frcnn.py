...
shared_layers = TimeDistributed(nn.nn_base(trainable=True))(img_input)
num_channels = shared_layers.shape[-1]

nb_filter = 40
clstm = Lambda(lambda x: clstm(x,num_channels,nb_filter,3), output_shape=(None,None,None,nb_filter))
shared_layers = clstm(shared_layers)

# define the RPN, built on the base layers
num_anchors = len(C.anchor_box_scales) * len(C.anchor_box_ratios)
rpn = TimeDistributed(nn.rpn(num_anchors))(shared_layers)
